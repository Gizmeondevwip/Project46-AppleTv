...
class HomeViewController: UIViewController {
    ...
    @IBOutlet weak var videoNameLabel: UILabel!{
        didSet{
            self.videoNameLabel.textColor = ThemeManager.currentTheme().UIImageColor
        }
    }
    @IBOutlet weak var descriptionWidth: NSLayoutConstraint!
    @IBOutlet weak var producerName: UILabel!
    @IBOutlet weak var videoDescrioptionLabel: UILabel!
...
extension HomeViewController: CommonVideoTableViewCellDelegate  {
  ...
  func didFocusPartner(passModel: VideoModel) {
    ...
    func didSelectDianamicVideos(passModel: VideoModel?) {
            ...
            newsVC.newsId = passModel?.show_id
            newsVC.isFromUpcomingEvents = false
            self.present(newsVC, animated: true, completion: nil)
        }
        else if passModel?.type == "UPCOMING_EVENT"{
            let newsVC =  self.storyboard?.instantiateViewController(withIdentifier: "NewsVC") as! NewsViewController
//            newsVC.eventId = passModel?.event_id
            newsVC.isFromUpcomingEvents = true
           ...
           livePlayngViewController.livelink = (passModel?.live_link)!
            ...
            let livePlayngViewController =  self.storyboard?.instantiateViewController(withIdentifier: "LiveVC") as! LivePlayingViewController
           livePlayngViewController.livelink = (passModel?.live_url)!
            self.present(livePlayngViewController, animated: true, completion: nil)
        }
        else{
            let videoDetailView =  self.storyboard?.instantiateViewController(withIdentifier: "videoDetail") as! VideoDetailsViewController
...
//            \n\n\(resolution)\n\nProducer: \(producer)\n\nYear: \(year)\n\nThemes: \(categories.dropLast(1))"
        ...
        if let description = passModel.synopsis{
            videoDescrioptionLabel.text = "\(description)"
//            descriptionWidth.constant = view.frame.width / 3 + 50
            videoDescrioptionLabel.numberOfLines = 4
        }
        else{
    ...
    func didFocusDianamicVideos(passModel: VideoModel) {
        ...
        if let year = passModel.year{
            yearLabel.text = year
        }
        else{
            yearLabel.text?.removeAll()
        }
        if let description = passModel.synopsis{
            videoDescrioptionLabel.text = "\(description)"
            videoDescrioptionLabel.numberOfLines = 4
